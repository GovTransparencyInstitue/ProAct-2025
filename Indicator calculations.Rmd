...
calc_transparency_indicators <- function(dt) {
  ...
  #   - Most transparency indicators: missing input column => indicator stays 0
  #   - Benford: missing input column => NA (special case, NOT part of index)
  # -------------------------
  dt[, `:=`(
    ind_tr_proc_type       = 0L,
    ind_tr_buyer_id        = 0L,
    ...
    ind_tr_prod_code       = 0L,
    ind_tr_benford         = NA_integer_   # mirrors ind_corr_benfords if present
  )]
  ...
  #   - If source column exists: mirror it (keep its NAs)
  #   - If missing: stays NA_integer_
  # -------------------------
  if ("ind_corr_benfords" %in% names(dt)) {
    dt[, ind_tr_benford := as.integer(ind_corr_benfords)]
  }
...
calc_competition_indicators <- function(dt) {
  ...
  #   - Only where bid count reliably recorded: ind_tr_bids_nr_missing == 0
  # -------------------------
  eligible_open_reliable <- rep(FALSE, nrow(dt))
  if ("tender_proceduretype" %in% names(dt) && "ind_tr_bids_nr_missing" %in% names(dt)) {
    proc_clean <- toupper(trimws(as.character(dt[["tender_proceduretype"]])))
    open_only <- !is.na(proc_clean) & proc_clean == "OPEN"
    reliable <- !is.na(dt[["ind_tr_bids_nr_missing"]]) & dt[["ind_tr_bids_nr_missing"]] == 0L
    eligible_open_reliable <- open_only & reliable
  }

  # -------------------------
  # 1) Average number of bids (mirror bids count), eligible rows only
  # -------------------------
  if ("ind_comp_bids_count" %in% names(dt)) {
    dt[eligible_open_reliable == TRUE, ind_comp_avg_bids := fifelse(
  ...
  # -------------------------
  # 2) Single bidding (invert corrected single-bid indicator), eligible rows only
  # Rule:
  #   ind_corr_singleb == 0 -> 1
  #   ind_corr_singleb == 1 -> 0
  #   else -> NA
  # -------------------------
  if ("ind_corr_singleb" %in% names(dt)) {
    dt[eligible_open_reliable == TRUE, ind_comp_single_bid := fifelse(
      is.na(ind_corr_singleb),
      NA_integer_,
      fifelse(ind_corr_singleb %in% c(0L, 1L),
              as.integer(1L - as.integer(ind_corr_singleb)),
              NA_integer_)
    )]
  }
